...
module TestHelpers
  def stop_test_redis
    ...
    sleep 5
    `rm -f #{Mobilize::Base.root}/spec/dump.rdb #{Mobilize::Base.root}/spec/dump-cluster.rdb`
  end

  def restart_test_redis
    stop_test_redis
    if !system("which redis-server")
      raise "** can't find `redis-server` in your path, you need redis to run Resque and Mobilize"
    end
    "redis-server #{Mobilize::Base.root}/spec/redis-test.conf".bash
  end
  ...
  def build_test_runner(user_name)
    set_test_env
  ...
  def load_fixture(name)
    #assume yml, check
    yml_file_path = "#{Mobilize::Base.root}/spec/fixtures/#{name}.yml"
    standard_file_path = "#{Mobilize::Base.root}/spec/fixtures/#{name}"
  ...
  def write_fixture(fixture_name, target_url, options={})
    u = owner_user
    fixture_raw = load_fixture(fixture_name)
    if options['replace']
  ...
  def confirm_expected_jobs(expected_fixture_name,time_limit=600)
    #clear all failures
    ::Resque::Failure.clear
    jobs = {}
    jobs['expected'] = load_fixture(expected_fixture_name)
